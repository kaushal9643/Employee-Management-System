...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await sendEmail({
      ...
      body: `<div style="max-width: 600px;">
              <h2>Hi ${employee.firstName}, 👋</h2>
              <p>You have checked in today at ${attendance?.checkIn?.toLocaleTimeString()}</p>
              <p>Please make sure to check-out.</p>
            </div>`
    });


    // fallback auto checkout logic
    if (!attendance.checkOut) {
      attendance.checkOut = new Date(attendance.checkIn).getTime() + 4 * 60 * 60 * 1000;
      attendance.workingHours = 4;
      attendance.dayType = "Half Day";
  ...
  async ({ event }) => {
    ...
    if (leaveApplication.status === "PENDING") {
      ...
      await sendEmail({
        ...
        body: `<div>
                <h2>Hi Admin 👋</h2>
                <p>${employee.firstName} applied for leave.</p>
              </div>`
      });
    }
  }
      ...
      await step.run("send-reminder-emails", async () => {
        ...
        const emailPromises = absentEmployees.map((emp) => {
          return sendEmail({
            ...
            body: `<p>Hi ${emp.firstName}, please mark attendance.</p>`
          });
        });
